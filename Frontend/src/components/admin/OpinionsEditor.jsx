...
export default function OpinionsEditor() {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState('articles'); // Only one 'tab' really, but keeps structure consistent
  ...
  const fetchData = async () => {
    try {
      const snap = await getDoc(doc(db, 'opinions', 'main'));
      if (snap.exists()) {
        setData(snap.data());
      } else {
        setData({});
      ...
      console.error("Error fetching opinions data:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddArticle = async () => {
    ...
    await saveData({ articles: [...list, newItem.trim()] });
    setNewItem('');
  };
  ...
  const handleUpdateArticle = async (index) => {
    ...
    await saveData({ articles: updatedList });
    setEditIndex(null);
    setEditValue('');
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">Manage Opinions</h2>
      ...
      <div className="mb-4">
          ...
          <p className="text-sm text-gray-500 mb-2">Format: Title (Source, Year)</p>
            ...
            {(data.articles || []).map((item, index) => (
                <div key={index} className="flex items-center gap-2 p-3 border rounded bg-gray-50 hover:bg-white transition-colors group">
                    {editIndex === index ? (
                        ...
                        <>
                            <span className="flex-1 block text-gray-800 text-sm">{item}</span>
